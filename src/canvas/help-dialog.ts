...
export function openHelpDialog(title: string, shortcuts: HelpShortcut[]) {
  const rows = shortcuts
    ...
    .join("")
  new Dialog({
    content: `<div style="max-height:60vh;overflow:auto;padding: 4px 12px 4px 16px;"><table style="width:100%;border-collapse:collapse;font-size:13px;">${rows}</table></div>`,
    title,
    width: "480px",
  })
}
