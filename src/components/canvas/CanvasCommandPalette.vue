...
const commandItems = computed<PaletteItem[]>(() => {
  ...
  return [
    ...
    { id: "cmd:zoom-in", kind: "command", title: props.t("toolbarZoomIn"), keywords: ["zoom in", "放大"], shortcut: "Shift++", run: () => e.zoomIn() },
    { id: "cmd:zoom-out", kind: "command", title: props.t("toolbarZoomOut"), keywords: ["zoom out", "缩小"], shortcut: "Shift+-", run: () => e.zoomOut() },
    ...(e.zoomToActualSize ? [{ id: "cmd:zoom-actual", kind: "command" as const, title: props.t("toolbarZoomActual"), keywords: ["actual", "实际"], shortcut: "Shift+0", run: () => e.zoomToActualSize!() }] : []),
