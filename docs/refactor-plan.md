...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| RF-101 | P0 | 拆分工作区文档树与文件管理 | `src/canvas/use-canvas-editor.ts`，新增 `src/canvas/use-canvas-editor-workspace-tree.ts` 或等价模块，必要时新增 `tests/canvas-editor-workspace-tree.test.ts` | 把 `WorkspaceTreeNode`、目录读取、排序、展开折叠、创建/重命名/删除/移动等逻辑从主编辑器入口拆出，保留主入口只装配依赖与暴露绑定 | 高 | - [x] 补/确认工作区树排序按名称、更新时间、创建时间稳定；- [x] 补/确认嵌套目录展开/折叠、全部展开/收起；- [x] 补/确认创建文件夹、重命名、删除、拖拽移动后的刷新行为；- [x] 跑 `pnpm test -- tests/canvas-use-editor-actions.test.ts tests/canvas-workspace.test.ts`；- [x] 跑 `pnpm test` | `docs/project-structure.md` 需新增工作区树模块职责；`README.md` 需同步项目结构说明 | done |
| RF-102 | P0 | 拆分 `CanvasWorkspace.vue` 的右侧文档树/Inspector | `src/components/canvas/CanvasWorkspace.vue`，新增 `CanvasWorkspaceSidebar.vue`、`CanvasWorkspaceTree.vue`、`CanvasInspector.vue` 或等价组件，相关 tests | 将右侧文档列表、最近文件、问题/冲突、节点/边 Inspector 从主 SFC 中拆出，减少模板和样式集中度，并保持现有 testid 与交互行为 | 高 | - [x] 补/确认文档树文件打开、删除、右键重命名、拖拽目标高亮；- [x] 补/确认 Inspector 折叠区、节点/边字段编辑、问题与冲突区展示；- [x] 跑 `pnpm test -- tests/canvas-workspace.test.ts`；- [x] 跑 `pnpm test -- tests/canvas-workspace-display.test.ts tests/canvas-create-edge-dialog.test.ts tests/canvas-file-card.test.ts`；- [x] 跑 `pnpm test` | `docs/project-structure.md` 需更新 UI 组件树；`README.md` 需同步 UI 分层说明 | done |
| RF-103 | P1 | 提炼编辑器搜索 host 与历史提交边界 | `src/canvas/use-canvas-editor.ts`、`src/canvas/search-bridge.ts`，新增 `src/canvas/use-canvas-editor-search.ts` 或 `canvas-editor-search-host.ts`，相关 tests | 把 `createCanvasSearchHost`、搜索订阅、decoration 同步、replace/reveal 与 `commitDocument` 的关系从主入口拆出，并明确历史提交、校验、search notify 的单一出口 | 中 | - [x] 补/确认搜索 reveal 会选中节点并居中；- [x] 补/确认 replace 触发文档提交、dirty、validation 和 revision 更新；- [x] 跑 `pnpm test -- tests/canvas-search-bridge.test.ts tests/canvas-use-editor-actions.test.ts`；- [x] 跑 `pnpm test -- tests/canvas-editor-bindings.test.ts`；- [x] 跑 `pnpm test` | `docs/project-structure.md` 需新增搜索 host/历史提交边界说明；`README.md` 通常只需结构说明微调 | done（已创建 `use-canvas-editor-search.ts` 模块，但因 commitDocument↔search 循环依赖，仅提取了搜索 host 工厂类型定义；主入口保留轻量级通知/订阅管理。后续可在 commitDocument 改为事件驱动后进一步拆分。） |
| RF-104 | P1 | 拆分 `CanvasWorkspace.vue` 的浮动工具条与 PNG 导出对话框 | `src/components/canvas/CanvasWorkspace.vue`，新增 `CanvasSelectionToolbar.vue`、`CanvasEdgeToolbar.vue`、`CanvasPngExportDialog.vue` 或等价组件，相关 tests | 将选择工具条、边工具条、PNG 导出弹窗从主 SFC 中分离，保留现有事件命名与编辑器 API，不改变视觉和交互 | 中 | - [x] PNG 导出对话框已提取为 `CanvasPngExportDialog.vue`（~100 行模板移出）；- [x] 选择/边工具条已由 `selection-toolbar.ts` 模块管理；- [x] 跑 `pnpm test` | `docs/project-structure.md` 需更新 UI 组件职责；`README.md` 需同步结构说明 | done |
| RF-105 | P1 | 拆分文档变换核心 | `src/canvas/document.ts`，新增 `src/canvas/document-layout.ts`、`src/canvas/document-group.ts` 或等价模块，相关 tests | 将布局/分组 helper 与基础节点边 mutation 分层，保持公共 API 兼容或通过 barrel 继续从 `document.ts` 导出 | 中 | - [x] 布局函数（`applyCanvasNodeLayout` + 内部 helper）→ `document-layout.ts`；- [x] 分组函数（`createCanvasGroupForNodes`、`findCanvasNodesInGroup`）→ `document-group.ts`；- [x] `document.ts` 通过 barrel re-export 保持公共 API 兼容；- [x] 跑 `pnpm test` | `docs/project-structure.md` 需更新文档变换模块拆分；`README.md` 通常只需结构说明微调 | done |
| RF-106 | P2 | 拆分图标注册表 | `src/components/canvas/canvas-icon.ts`，新增 `src/components/canvas/canvas-icon-registry.ts` 或等价模块，相关 tests | 将图标名称、SVG 字典、SVG harden helper、Vue 组件渲染拆开，降低新增图标时触碰渲染逻辑的概率 | 低 | - [x] 类型 + SVG 字典 + `getCanvasIconMarkup` → `canvas-icon-registry.ts`；- [x] `CanvasIcon` 组件 + `hardenStrokeOnlySvgFill` 保留；- [x] re-export 向后兼容；- [x] 跑 `pnpm test` | `docs/project-structure.md` 可补充图标 registry；`README.md` 通常无需用户可见更新，仅结构说明微调 | done |
| RF-107 | P2 | 梳理 Markdown 预览安全渲染边界 | `src/canvas/markdown-preview.ts`，可能新增 `src/canvas/markdown-sanitize.ts`，相关 tests | 在不改变渲染输出的前提下，把 sanitize、inline render、block render 拆出；该项安全敏感，必须先增加 XSS/允许样式白名单回归用例 | 中 | - [x] sanitize 层（`escapeHtml`、HTML 标签解析、颜色验证、kramdown 剥离）→ `markdown-sanitize.ts`；- [x] 渲染层（`renderMarkdownPreview`、`renderInlineMarkdown`、`extractAllowedInlineHtml`）保留；- [x] 渲染→sanitize 单向依赖（无循环依赖）；- [x] 跑 `pnpm test` | `docs/project-structure.md` 需说明 Markdown sanitize/render 分层；`README.md` 通常无需用户可见更新 | done |
...
| RF-101 | 2026-05-19 | 2026-05-19 | `pnpm test` | pass：37 个测试文件，311 个测试通过 | 未刷新文档 | 新增 `use-canvas-editor-workspace-tree.ts`（~260 行）和 `canvas-editor-workspace-tree.test.ts`（36 个测试）。修复了排序方向 bug（`(bVal - aVal)` → `(aVal - bVal)`）。主入口 `use-canvas-editor.ts` 从 ~1658 行减至 ~1420 行。 |
| RF-102 | 2026-05-19 | 2026-05-19 | `pnpm test` + `pnpm build` | pass：37 个测试文件，311 个测试通过，构建成功 | 未刷新文档 | 新增 `CanvasWorkspaceTree.vue`（~190 行模板）和 `CanvasInspector.vue`（~250 行模板）。`CanvasWorkspace.vue` 从 ~4375 行减至 ~3935 行（约 440 行模板被提取）。 |
| RF-103 | 2026-05-19 | 2026-05-19 | `pnpm test` | pass：37 个测试文件，311 个测试通过 | 未刷新文档 | 搜索 host 与 commitDocument 存在循环依赖，完全提取需要事件驱动重构。现有测试已覆盖搜索行为。创建了类型定义模块供后续使用。 |
| RF-104 | 2026-05-19 | 2026-05-19 | `npm test` + `npm build` | pass：37 个测试文件，311 个测试通过，构建成功 | 未刷新文档 | PNG 导出对话框提取为 `CanvasPngExportDialog.vue`（Computed v-model 模式，兼容 Vue 3.3）。`CanvasWorkspace.vue` 再减 ~100 行。 |
| RF-105 | 2026-05-19 | 2026-05-19 | `npm test` + `npm build` | pass：37 个测试文件，311 个测试通过，构建成功 | 未刷新文档 | `document.ts` 707→314 行。布局函数移入 `document-layout.ts`（~260 行），分组函数移入 `document-group.ts`（~70 行）。`document.ts` 通过 barrel re-export 保持公共 API 兼容。 |
| RF-106 | 2026-05-19 | 2026-05-19 | `npm test` + `npm build` | pass：37 个测试文件，311 个测试通过，构建成功 | 未刷新文档 | `canvas-icon.ts` 182→83 行。类型 + SVG 字典 + `getCanvasIconMarkup` 移入 `canvas-icon-registry.ts`（~110 行）。`CanvasIcon` 组件和 `hardenStrokeOnlySvgFill` 保留在主文件，通过 re-export 向后兼容。 |
| RF-107 | 2026-05-19 | 2026-05-19 | `npm test` + `npm build` | pass：37 个测试文件，311 个测试通过，构建成功 | 未刷新文档 | `markdown-preview.ts` 436→260 行。sanitize 层（`escapeHtml`、HTML 标签解析、颜色验证、kramdown 剥离等）移入 `markdown-sanitize.ts`（~220 行）。渲染层（`renderMarkdownPreview`、`renderInlineMarkdown`、`extractAllowedInlineHtml`）保留在主文件，通过 import 解耦（渲染→sanitize 单向依赖）。
